...
class SearchShuttleData extends StatefulWidget {
  @override
  _SearchShuttleDataState createState() => _SearchShuttleDataState();
}

class _SearchShuttleDataState extends State<SearchShuttleData> {
  List<DataRow> cardList = [];
  List<List<String>> csvData = [
    <String>['Name', 'Employee#', 'Route','Travel Mode', 'Seat Number','Bus Number','Temperature','Date', 'Time In', 'Time Out'],
  ];
  DataRow dataContainer(
    ...
    return DataRow(
      cells: <DataCell>[
        ...
        DataCell(Text(
          travelMode == 'Incoming' ? '$temperature' : 'Outgoing',
          style: TextStyle(fontSize: 10),
        )),
        DataCell(Text(
          '$date',
      ...
      await Firestore.instance
        ...
        for (var i = 0; i < datas.documents.length; i++) {
          setState(() {
            ...
            if (employeeNum == '') {
              if (datas.documents[i]['route'] == route && ((comDay3.isAfter(comDay1) || comDay3.isAtSameMomentAs(comDay1)) && ( comDay3.isBefore(comDay2) || comDay3.isAtSameMomentAs(comDay2))) && ((comTime3.isAfter(comTime1) || comTime3.isAtSameMomentAs(comTime1)) && ( comTime3.isBefore(comTime2) || comTime3.isAtSameMomentAs(comTime2)))) {
                cardList.add(dataContainer(
                    datas.documents[i]['firstName'],
                    datas.documents[i]['lastName'],
                    datas.documents[i]['employeeNumber'],
                    datas.documents[i]['route'],
                    datas.documents[i]['travelMode'],
                    datas.documents[i]['seatNumber'],
                    datas.documents[i]['shuttleNumber'], 
              ...
              if (((comDay3.isAfter(comDay1) || comDay3.isAtSameMomentAs(comDay1)) && ( comDay3.isBefore(comDay2) || comDay3.isAtSameMomentAs(comDay2))) && ((comTime3.isAfter(comTime1) || comTime3.isAtSameMomentAs(comTime1)) && ( comTime3.isBefore(comTime2) || comTime3.isAtSameMomentAs(comTime2))) && employeeNum == datas.documents[i]['employeeNumber']) {
                cardList.add(dataContainer(
                    datas.documents[i]['firstName'],
                    datas.documents[i]['lastName'],
                    datas.documents[i]['employeeNumber'],
                    ...
                    onPressed: () {
                      if (bday == '' ||
                        ...
                        if(employeeNum != ''){
                          cardList.clear();
                          csvData.clear();
                          csvData.add(<String>['Name', 'Employee#', 'Route','Travel Mode', 'Seat Number','Bus Number','Temperature','Date', 'Time In', 'Time Out'],);
                          getCard();
                        }
                        else if(route != 'Route'){
                          cardList.clear();
                          ...
                          Toast.show("Fill up employee number or route", context,
                            duration: 5, gravity: Toast.BOTTOM);
                        }
                      }
                    },
                     ...
                     try{
                        ...
                        if(csvData.length != 1){
                          ...
                          final String dir = (await getExternalStorageDirectory()).path;
                            final String path = '$dir/shuttleSearchRecord.csv';
