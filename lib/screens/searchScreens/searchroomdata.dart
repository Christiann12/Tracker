...
class SearchRoomData extends StatefulWidget {
  @override
  _SearchRoomDataState createState() => _SearchRoomDataState();
}

class _SearchRoomDataState extends State<SearchRoomData> {
  List<List<String>> csvData = [
    <String>['Name', 'Employee#', 'Room Name', 'Date', 'Time In', 'Time Out'],
  ];
      ...
      await Firestore.instance
        ...
        for (var i = 0; i < datas.documents.length; i++) {
          setState(() {
            ...
            if (employeeNum == '') {
              if (datas.documents[i]['roomName'] == roomName && ((comDay3.isAfter(comDay1) || comDay3.isAtSameMomentAs(comDay1)) && ( comDay3.isBefore(comDay2) || comDay3.isAtSameMomentAs(comDay2)))) {
                
                cardList.add(dataContainer(
                    datas.documents[i]['firstName'],
                    datas.documents[i]['lastName'],
                    datas.documents[i]['employeeNumber'],
                    datas.documents[i]['roomName'],
              ...
              if (datas.documents[i]['roomName'] == roomName && ((comDay3.isAfter(comDay1) || comDay3.isAtSameMomentAs(comDay1)) && ( comDay3.isBefore(comDay2) || comDay3.isAtSameMomentAs(comDay2))) && employeeNum == datas.documents[i]['employeeNumber']) {
                cardList.add(dataContainer(
                    datas.documents[i]['firstName'],
                    datas.documents[i]['lastName'],
                    datas.documents[i]['employeeNumber'],
                  ...
                  child: FlatButton(
                    ...
                    onPressed: () {
                      if (roomName == 'Room Name' ||
                        ...
                        csvData.clear();
                        csvData.add(<String>['Name', 'Employee#', 'Room Name', 'Date', 'Time In', 'Time Out'],);
                        cardList.clear();
                        getCard();
                      }
                    },
                     ...
                     try{
                        ...
                        if(csvData.length != 1){
                          ...
                          final String dir = (await getExternalStorageDirectory()).path;
                            final String path = '$dir/conferenceRoomSearchRecord.csv';
